# AES (Advanced Encryption Standard) Algorithm in python
# Author: M-Taghizadeh 
...
def hext_to_string(hex):
    string = ""
    i = 0
    while i<len(hex):
        tmp_hex = hex[i] + hex[i+1]
        string += chr(int(tmp_hex, 16))
        i += 2
    return string
...
if __name__ == "__main__":
    ...
    # INPUT STRING 
    key = "_The_Secret_Key_"
    plaintext = "TheSecretMessage"
    print(f'Key: {key}')
    ...
    print("------------------------------------------")
    print("ENCRYPTION")
    cipher_bytes = crypt.cipher(plaintext_bytes)
    cipher_hex = cipher_bytes.hex()
    cipher = hext_to_string(cipher_hex)
    print(f'Cipher Bytes: {cipher_bytes}')
    ...
    print("------------------------------------------")
    print("DECRYPTION")
    plaintext_bytes = crypt.inv_cipher(cipher_bytes)
    print(f'Decrypted Plain text: {plaintext_bytes.decode("utf-8")}')
